...
const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, navItems }) => (
  ...
  >
    ...
    <div className="pt-4 pb-4 border-t border-gray-200 dark:border-gray-700">
      <div className="px-5">
        <Link href="/sign-in" className="w-full text-sm font-medium text-gray-900 dark:text-white hover:text-orange-500">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex items-center space-x-1 bg-gray-100/50 dark:bg-gray-800/50 p-1 rounded-full">
            {navItems.map((item) => (
              <NavLink key={item} href={`#${item.toLowerCase()}`} isActive={activeLink === item}>
                {item}
              </NavLink>
            ))}
          </nav>
          <div className="hidden md:block">
            <Link href="/sign-in" className="text-sm font-medium text-gray-900 dark:text-white hover:text-orange-500">
              <Button variant="outline">Login Now</Button>
            </Link>
          </div>
